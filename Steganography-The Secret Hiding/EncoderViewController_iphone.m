//
//  EncoderViewController_iphone.m
...
@interface EncoderViewController_iphone ()

@end

@implementation EncoderViewController_iphone

@synthesize image,message,imgdict,user_image, textLimit,charDisplay, pwdFlag, password,onOffpassword,photosInstruction;
...
- (IBAction)create_stego:(id)sender
{
    ...
    else if (pwdFlag==YES && [[password text] isEqualToString:@"" ])
    {
        ...
        //**********************************************************
        unsigned long byteIndex = (int)(width)*(height/2)*4+(int)(width/2)*4;
        //int byteIndex=0;
        ...
        UIAlertView *alert1=[[UIAlertView alloc] initWithTitle:nil message:@"Message Hidden in image" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        [alert1 show];
...
- (IBAction)browseDocs:(id)sender
{
    ...
    NSArray *allFiles = [filemgr contentsOfDirectoryAtPath:docPath error:NULL];
    for (NSString *fileName in allFiles)
    {
        if ([[fileName pathExtension] isEqualToString:@"txt"])
        {
            ...
            NSString *path=[NSString stringWithFormat:@"%@%@",docPath,fileName];
        ...
        /*if ([[fileName pathExtension] isEqualToString:@"pdf"])
        {
            NSLog(@"%@",fileName);
            //do whatever you want to do with the path
            NSString* content = [NSString stringWithContentsOfFile:[NSString stringWithFormat:@"%@%@.pdf",docPath,fileName]
                                                          encoding:NSUTF8StringEncoding
                                                             error:NULL];
            NSLog(@"%@",content);
        }*/
...
{
    //UIImage *img=[imgdict objectForKey:UIImagePickerControllerEditedImage];
